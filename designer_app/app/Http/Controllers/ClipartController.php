...
class ClipartController extends Controller
{
    ...
    public function __construct(User $user, UserMeta $usermeta, Post $post, PostMeta $postmeta, Setting $setting, Request $request)
    {
        ...
        $this->post_type = 'clipart';
        $this->view      = 'admin.cliparts';
        $this->single    = 'Clipart';
        $this->label     = 'Cliparts';
    ...
    public function index()
    {
        ...
        $data['categories'] = $this->post
                                   ->where('post_type', 'clipart-category')
                                   ->get()
                                   ->pluck('post_title', 'id')
                                   ->toArray();
    ...
    public function add()
    {
        ...
        if( Input::get('_token') )
        {
            ...
            if( $post->save() ) {
                $this->postmeta->update_meta($post->id, 'image', Input::get('image'));
