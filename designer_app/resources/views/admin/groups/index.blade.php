...
<div class="portlet light bordered margin-top-10">
    ...
    <table class="table table-hover table-striped">
        ...
        <tbody>
            ...
            <tr class="tr-row-md has-row-actions">
                ...
                <td>
                    <h5 class="sbold no-margin"><a href="{{ URL::route($view.'.edit', $row->id) }}">{{ ucwords($row->post_title) }}</a></h5>
                ...
                <td>
                    <?php $permissions = json_decode($row->post_content, true); ?>
                    @if($permissions)
                        {{ implode(', ', array_keys( $permissions )) }}
                    @endif
                </td>
                <td class="text-right">{{ number_format($user->where('group', $row->post_name)->count()) }}</td>
    ...
    @if( ! count($rows) )
        <div class="well">No groups found.</div>
    @endif
